...
function [A,x_0,y_0,sigma_x,sigma_y,differenze] = gaussianFit(image,max,centro_x,centro_y,var_x,var_y)
    ...
    for i=1:m
        x = mod(i ,  dimx);
        y = floor(i/dimx);
        immagine(i) = valutaPunto(A,x_0,y_0,sigma_x,sigma_y,x,y);
    end
    predizione = reshape(immagine,dimx,dimy);
    figure(2);
    mesh(predizione);
    ...
    for j=1:iterazioni
        ...
        for i=1:m
            x = mod(i ,  dimx);
            y = floor(i/dimx);
            test = valutaPunto(A,x_0,y_0,sigma_x,sigma_y,x,y);
            %tutto il problema e` qui! -> il round e` una vera merda
            differenze(i) = round(double(img(i)) - test);
