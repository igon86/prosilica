...
void scatterImage(unsigned char* matrix,int dim,unsigned char* partition){
	...
	if(my_rank == EMITTER){	
		for (i=PS;i<p;i++){
			MPI_Send(matrix + (i-PS)*ppw, ppw, MPI_UNSIGNED_CHAR, i,IMAGE, MPI_COMM_WORLD);
		}
	}else{
		if (my_rank >= PS){
		MPI_Recv(partition,ppw,MPI_UNSIGNED_CHAR,EMITTER,IMAGE,MPI_COMM_WORLD,&status);	
		}
	}	
}
...
void reduceMatrix(double* matrix,int dim,double* partition){
	...
	if(my_rank == COLLECTOR){
		...
		for (i=PS;i<p;i++){
			MPI_Recv(buffer,dim,MPI_DOUBLE,i,MATRIX,MPI_COMM_WORLD,&status);
			 for (j=0;j<dim;j++){
		...
		if(my_rank >= PS){
			MPI_Send(partition, dim, MPI_DOUBLE, COLLECTOR, MATRIX, MPI_COMM_WORLD);
		}
	}	
	free(buffer);
}