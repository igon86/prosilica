...
#apro il file
echo "echo DATAPARALLEL" >> test.sh
...
do
	...
	#COMPILO
	echo "mpicc $FLAGS -DDATA_PARALLEL -Wall -pedantic  -I/home/parallel/lottarin/include -lgsl -lgslcblas -lm -ltiff ./data_parallel.c ./fit.c ./image.c ./macro.c -L/home/parallel/lottarin/lib -o dataparallel" >> test.sh
	...
	do
		...
		do
			if [ $(echo $TAGNAME | grep MODULE ) ]; then
				echo "mpirun -np $i ./dataparallel $dim $dim >> ./TEMPI/dataparallel_$TAGNAME &" >> test.sh
				echo "./camera" >> test.sh
			else	
				echo "mpirun -np $i ./dataparallel $dim $dim >> ./TEMPI/dataparallel_$TAGNAME" >> test.sh
			fi
