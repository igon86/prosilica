 ...
 */
void postProcedure(gsl_matrix_view matrice, const gsl_vector_view vettore,
		   double *fit)
{
    ...
    gsl_linalg_LU_solve(&matrice.matrix, permutation, &vettore.vector,
			delta);
#if OLD
...
void computeGradient(double *M, int x, int y, double *results)
{
    ...
    M[0] = 1 / dexp;
    M[1] =
	(results[PAR_A] * (2 * x - 2 * results[PAR_X])) / (sig2x * dexp);
    M[2] =
	(results[PAR_A] * (2 * y - 2 * results[PAR_Y])) / (sig2y * dexp);
    M[3] =
...
/**
 Creates a new server socket and waits for clients. Once a connection is
 established the server socket is closed and the newly created socket
 ...
 */
void initialization(unsigned char *matrix, int width, int height,
		    double *fit)
{
...
#if DEBUG
    writeImage(matrix, (char *) "gaussiana.tiff", width, height);
#endif
    ...
    mask = createMask(matrix, width, height, max, min, CROP_PARAMETER);
